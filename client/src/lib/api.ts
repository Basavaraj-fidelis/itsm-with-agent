...
export const api = {
  ...
  // Analytics endpoints
  async getPerformanceInsights(deviceId: string) {
    try {
      return await apiClient.get(`/api/analytics/performance/insights/${deviceId}`);
    } catch (error) {
      console.warn('Performance insights failed, returning fallback data:', error);
      return {
        top_cpu_consumers: [],
  ...
  async getPerformancePredictions(deviceId: string) {
    try {
      return await apiClient.get(`/api/analytics/performance/predictions/${deviceId}`);
    } catch (error) {
      console.warn('Performance predictions failed, returning empty array:', error);
      return [];
    }
  },
  ...
  async getPerformanceOverview() {
    try {
      return await apiClient.get("/api/analytics/performance/overview");
    } catch (error) {
      console.warn('Performance overview failed, returning fallback data:', error);
      return {
        totalDevices: 0,
        onlineDevices: 0,
        avgCpuUsage: 0,
  ...
  async getAdvancedDeviceAnalytics(deviceId: string) {
    try {
      return await apiClient.get(`/api/analytics/device/${deviceId}/advanced`);
    } catch (error) {
      console.warn('Advanced device analytics failed, returning fallback data:', error);
      return {
        performance_trends: {
          cpu_trend: [],
          memory_trend: [],
