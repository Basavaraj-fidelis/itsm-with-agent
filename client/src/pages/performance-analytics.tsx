...
export default function PerformanceAnalytics() {
  ...
  const { data: devices, isLoading: devicesLoading, isError: devicesError, refetch: refetchDevices } = useQuery({
    queryKey: ["devices"],
  ...
  const handleExport = () => {
    ...
    if (csvData) {
      const csv = [
        ...
        ...csvData.map(row => `${row.hostname},${row.status},${row.cpu_usage},${row.memory_usage},${row.disk_usage},${row.last_seen}`)
      ].join('\n');
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <MetricCard
          ...
          change={Math.random() > 0.5 ? -Math.floor(Math.random() * 2) : Math.floor(Math.random() * 3)}
          icon={AlertTriangle}
          color="text-red-500"
        ...
        <MetricCard
          ...
          change={Math.random() > 0.5 ? Math.floor(Math.random() * 10) - 5 : -Math.floor(Math.random() * 5)}
          icon={Cpu}
          color={avgMetrics.cpu > 80 ? "text-red-500" : avgMetrics.cpu > 60 ? "text-yellow-500" : "text-green-500"}
        ...
        <MetricCard
          ...
          change={Math.random() > 0.5 ? Math.floor(Math.random() * 8) - 4 : -Math.floor(Math.random() * 6)}
          icon={MemoryStick}
          color={avgMetrics.memory > 85 ? "text-red-500" : avgMetrics.memory > 70 ? "text-yellow-500" : "text-green-500"}
      ...
      <Tabs defaultValue="overview" className="space-y-6">
              ...
              <div className="space-y-4">
                {['CPU', 'Memory', 'Disk'].map((resource) => {
                  const metrics = onlineDevices.map(d => {
                    const key = resource.toLowerCase() === 'disk' ? 'disk_usage' : `${resource.toLowerCase()}_usage`;
                  ...
                  return (
                    <div key={resource} className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs text-muted-foreground">
                        ...
                        <span className={critical > 0 ? "text-red-600" : "text-green-600"}>
                          {critical} critical
                        </span>
                      </div>
                    </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium text-yellow-600">
                      {onlineDevices.filter(d => 
                        (parseFloat(d.latest_report?.cpu_usage || '0') >= 70 && parseFloat(d.latest_report?.cpu_usage || '0') < 90) ||
                        (parseFloat(d.latest_report?.memory_usage || '0') >= 70 && parseFloat(d.latest_report?.memory_usage || '0') < 90)
                      ).length}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm">Critical State</span>
                    <span className="text-sm font-medium text-red-600">{criticalDevices.length}</span>
                  </div>
                </div>
              </div>
            </ChartCard>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {['CPU', 'Memory', 'Disk'].map((metric) => {
              const sortedDevices = [...onlineDevices].sort((a, b) => {
                const key = metric.toLowerCase() === 'disk' ? 'disk_usage' : `${metric.toLowerCase()}_usage`;
                return parseFloat(b.latest_report?.[key] || '0') - parseFloat(a.latest_report?.[key] || '0');
              }).slice(0, 5);
                ...
                <ChartCard key={metric} title={`Top ${metric} Consumers`}>
                  <div className="space-y-3">
                    {sortedDevices.map((device, index) => {
                      const key = metric.toLowerCase() === 'disk' ? 'disk_usage' : `${metric.toLowerCase()}_usage`;
                      const usage = parseFloat(device.latest_report?.[key] || '0');
                      return (
                        <div key={device.id} className="flex items-center justify-between">
                          ...
                          <span className={`text-sm font-bold ${
                            usage > 90 ? 'text-red-600' : usage > 70 ? 'text-yellow-600' : 'text-green-600'
                          }`}>
                            {usage.toFixed(1)}%
                          </span>
                        </div>
            ...
            <CardContent>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <Card>
                          <CardContent className="pt-4">
                            ...
                            <div className="text-2xl font-bold">{device.latest_report?.cpu_usage || 0}%</div>
                            <Progress value={parseFloat(device.latest_report?.cpu_usage || '0')} className="mt-2" />
                          </CardContent>
                        </Card>
                        <Card>
                          <CardContent className="pt-4">
                            ...
                            <div className="text-2xl font-bold">{device.latest_report?.memory_usage || 0}%</div>
                            <Progress value={parseFloat(device.latest_report?.memory_usage || '0')} className="mt-2" />
                          </CardContent>
                        </Card>
                        <Card>
                          <CardContent className="pt-4">
                            ...
                            <div className="text-2xl font-bold">{device.latest_report?.disk_usage || 0}%</div>
                            <Progress value={parseFloat(device.latest_report?.disk_usage || '0')} className="mt-2" />
                          </CardContent>
                        </Card>
                        <Card>
                          <CardContent className="pt-4">
                  ...
                  {insights && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <Card>
                        ...
                        <CardContent>
                          <div className="space-y-2">
                            {insights.top_cpu_consumers?.slice(0, 8).map((process: any, index: number) => (
                              <div key={index} className="flex items-center justify-between text-sm">
                                <span className="truncate max-w-48" title={process.name}>{process.name}</span>
                      ...
                      <Card>
                        ...
                        <CardContent>
                          <div className="space-y-2">
                            {insights.top_memory_consumers?.slice(0, 8).map((process: any, index: number) => (
                              <div key={index} className="flex items-center justify-between text-sm">
                                <span className="truncate max-w-48" title={process.name}>{process.name}</span>
              ...
              {selectedDevice && !insights && !insightsError && (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm mt-1">Data will appear once the agent starts reporting</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="trends" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <ChartCard title="Performance Trends (24h)" actions={
              ...
              <div className="h-64 flex items-center justify-center text-muted-foreground">
                <div className="text-center">
                  ...
                  <p className="text-sm">Trends will appear after 24 hours of data collection</p>
                </div>
              </div>
            </ChartCard>
            ...
            <ChartCard title="Resource Utilization Patterns">
              <div className="space-y-4">
                <div className="text-sm text-muted-foreground mb-4">Peak usage patterns</div>
                  ...
                  return (
                    <div key={period} className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-medium">{usage.toFixed(0)}%</span>
                      </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-3 p-3 bg-red-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-red-700">
                          {criticalDevices.length} device(s) are operating at critical resource levels
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {avgMetrics.memory > 80 && (
                    <div className="flex items-start gap-3 p-3 bg-yellow-50 rounded-lg">
                      ...
                      <div>
                        <div className="font-medium text-yellow-900">Memory Upgrade Recommended</div>
                        <div className="text-sm text-yellow-700">
                          Average memory usage is {avgMetrics.memory.toFixed(1)}%. Consider upgrading RAM on high-usage devices.
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg">
                    ...
                    <div>
                      <div className="font-medium text-blue-900">Monitoring Active</div>
                      <div className="text-sm text-blue-700">
                        Performance monitoring is active on {onlineDevices.length} devices with {healthScore}% system health.
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  {['CPU', 'Memory', 'Storage'].map((resource) => {
                    const currentAvg = resource === 'CPU' ? avgMetrics.cpu : resource === 'Memory' ? avgMetrics.memory : avgMetrics.disk;
                    ...
                    return (
                      <div key={resource} className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Progress value={projected} className="flex-1 h-2 opacity-50" />
                        </div>
