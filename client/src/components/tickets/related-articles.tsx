...
export default function RelatedArticles({ ticket }: RelatedArticlesProps) {
  ...
  const fetchRelatedArticles = async () => {
    try {
      ...
      // Fetch articles with search terms
      const response = await api.get(`/api/knowledge-base?search=${encodeURIComponent(searchTerms)}&limit=5`);
      
      if (response.ok) {
        const data = await response.json();
        setArticles(data.slice(0, 3)); // Show only top 3 related articles
      }
    } catch (error) {
