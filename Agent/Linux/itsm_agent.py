...
#!/usr/bin/env python3
"""
Non-Intrusive ITSM Endpoint Agent - Linux Version
...
class ITSMAgent:
    ...
    def _validate_config(self):
        ...
        try:
            ...
            if validation_errors:
                ...
                critical_errors = [e for e in validation_errors if isinstance(e, str) and 'required' in e.lower()]
                if critical_errors:
    ...
    def _setup_logging(self):
        ...
        log_dir = Path('/var/log/itsm-agent') if os.path.exists('/var/log') else Path('logs')
    ...
    def start(self):
        ...
        self.logger.info("Starting Non-Intrusive ITSM Agent (Linux)...")
        self.running = True
...
class ITSMService(ServiceWrapper):
    ...
    def __init__(self):
        super().__init__('itsm-agent', 'ITSM Endpoint Agent')
        self.agent = None
    ...
    try:
        if len(sys.argv) > 1:
            ...
            if command in ['install', 'remove', 'start', 'stop', 'restart']:
                ...
                print("Usage: python3 itsm_agent.py [install|remove|start|stop|restart]")
                print("       python3 itsm_agent.py  (to run interactively)")
        else:
            # Running interactively or as systemd service
            agent = ITSMAgent()
            try:
                agent.start()
