...
import { patchComplianceService } from './patch-compliance-service';
...
router.get('/patch-compliance/dashboard', async (req, res) => {
  try {
    ...
    const isDatabaseError = error?.message?.includes('connection') || 
                           error?.message?.includes('timeout') ||
                           error?.code === 'ECONNREFUSED' ||
                           error?.code === 'ETIMEDOUT';

    const isTypeError = error?.message?.includes('operator does not exist') ||
