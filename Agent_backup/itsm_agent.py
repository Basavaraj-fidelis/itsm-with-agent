...
class ITSMAgent:
    ...
    def _load_config(self):
        ...
        defaults = {
            ...
            'api': {
                'base_url': os.getenv('ITSM_API_URL', 'http://localhost:5000'),
    ...
    def _validate_config(self):
        ...
        try:
            ...
            if validation_errors:
                ...
                # Don't exit in production, but log critical errors
            ...
            if hasattr(self, 'logger'):
                ...
                print(f"❌ {error_msg}")
            # Don't exit on validation errors in production, but ensure logging
        ...
        try:
            ...
            heartbeat_data = {
                ...
                'systemInfo': {
                    ...
                    'networkStatus': self.network_monitor.get_connectivity_status(),
                    'performanceBaseline': self.performance_baseline.get_baseline_status()
                }
            }

            response = self.api_client.send_heartbeat(heartbeat_data)
            if response and 'agentId' in response:
...
def main():
    ...
    try:
        if len(sys.argv) > 1:
            ...
            # Check if running as service by checking if we have a console
            is_service = False
            try:
                # Try to detect if running as Windows service
                import sys
                import os

                # Check various indicators of service mode
                if (not hasattr(sys.stdout, 'isatty') or 
                    not sys.stdout.isatty() or 
                    os.environ.get('NSSM_SERVICE_NAME') == 'ITSMAgent'):
                    is_service = True
            ...
            if is_service:
                # Running as Windows service - create a simple log to track this
                try:
                    with open('C:\\Program Files\\ITSM Agent\\service_start.log', 'a') as f:
                        import datetime
                        f.write(f"{datetime.datetime.now()}: Service starting...\n")
                except:
                    pass

                # Running as Windows service
                service = ITSMService()
                service.start_service()  # Use start_service directly
            else:
