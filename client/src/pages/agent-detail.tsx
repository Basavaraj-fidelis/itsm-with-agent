...
export default function AgentDetail() {
  ...
  const handleRemoteConnect = async (connectionType: string) => {
    ...
    if (lastReportTime) {
      const reportAge = Math.floor((Date.now() - new Date(lastReportTime).getTime()) / (1000 * 60));
      if (reportAge > 15) {
        const proceed = confirm(`Warning: Agent last reported ${reportAge} minutes ago. Connection may not work properly. Continue anyway?`);
        if (!proceed) return;
      }
    }
