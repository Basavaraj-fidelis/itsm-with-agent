...
class ApiClient {
  ...
  async getDevice(id: string) {
    const response = await this.get(`/api/devices/${id}`);
    return response;
  }

  async getDeviceAIInsights(id: string) {
    try {
      const response = await this.get(`/api/ai/insights/device/${id}`);
      return response;
    } catch (error) {
      console.warn(`AI insights not available for device ${id}:`, error);
      return null;
    }
  }

  async getAdvancedDeviceAnalytics(id: string) {
    try {
      const response = await this.get(`/api/analytics/device/${id}/advanced`);
      return response;
    } catch (error) {
      console.warn(`Advanced analytics not available for device ${id}:`, error);
      return null;
    }
  }
}